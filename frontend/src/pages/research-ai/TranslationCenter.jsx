...
import { Languages, ArrowRightLeft, Copy, CheckCheck } from 'lucide-react';
...
export default function TranslationCenter() {
  ...
  const translations = {
    hi: "यूरिया का प्रयोग तभी करें जब मिट्टी में पर्याप्त नमी हो, अधिमानतः हल्की सिंचाई के बाद।",
    pa: "ਯੂਰੀਆ ਦੀ ਵਰਤੋਂ ਉਦੋਂ ਹੀ ਕਰੋ ਜਦੋਂ ਮਿੱਟੀ ਵਿੱਚ ਕਾਫ਼ੀ ਨਮੀ ਹੋਵੇ, ਤਰਜੀਹੀ ਤੌਰ 'ਤੇ ਹਲਕੀ ਸਿੰਚਾਈ ਤੋਂ ਬਾਅਦ।",
    mr: "युरियाचा वापर तेव्हाच करा जेव्हा मातीत पुरेशी ओलावा असेल, शक्यतो हलक्या सिंचनानंतर."
  };

  const handleCopy = () => {
    navigator.clipboard.writeText(translations[targetLang]);
    setCopied(true);
      ...
      <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-0 items-stretch border border-gray-200 rounded-2xl overflow-hidden bg-gray-50/30">
          ...
          <div className="flex-1 p-6 relative group">
            ...
            <textarea
              className="w-full h-48 bg-transparent resize-none outline-none text-lg text-gray-800 placeholder-gray-400 font-medium leading-relaxed"
          ...
          <div className="flex items-center justify-center lg:w-16 bg-gray-100 lg:border-l lg:border-r border-gray-200">
            <button className="p-2 bg-white rounded-full shadow-sm border border-gray-200 hover:bg-gray-50 transition-colors z-10 -my-4 lg:my-0 lg:-mx-4">
              <ArrowRightLeft className="h-5 w-5 text-[#31572c] lg:rotate-0 rotate-90" />
            </button>
          </div>

          {/* Target Side */}
          <div className="flex-1 p-6 relative bg-white lg:bg-transparent">
            <div className="flex items-center justify-between mb-4">
              <select 
                value={targetLang}
            ...
            <div className="w-full h-48 text-lg text-gray-900 font-bold leading-relaxed">
              {sourceText ? translations[targetLang] : <span className="text-gray-300 font-medium">Translation will appear here...</span>}
            </div>
            
            {sourceText && (
              <div className="absolute bottom-6 right-6 flex items-center gap-1 text-[10px] font-black uppercase text-gray-400">
                <Languages className="h-3 w-3" /> AI Tuned
              </div>
            )}
          </div>
